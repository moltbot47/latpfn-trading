.PHONY: help install test lint coverage backup docker clean
...
test: ## Run all 218 tests
	python -m pytest tests/ -v --tb=short -x

lint: ## Run ruff linter on project files
	ruff check tests/ funding/ scripts/funding_dashboard.py scripts/backup.py scripts/health_monitor.py monitoring/logger.py

lint-fix: ## Auto-fix lint issues
	ruff check --fix tests/ funding/ scripts/funding_dashboard.py scripts/backup.py scripts/health_monitor.py monitoring/logger.py
